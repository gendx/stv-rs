...
do
    "${HYPERFINE_PATH}" \
        ...
        --warmup 1 \
        --parameter-list INDEX_COMMIT "${INDEX_COMMITS}" \
        "./bin/stv-rs-{INDEX_COMMIT} --arithmetic ${ARITHMETIC} --input ${INPUT} meek ${EQUALIZE} --parallel=custom --num-threads=${NUM_THREADS} > /dev/null"
    "${HYPERFINE_PATH}" \
        --style color \
        --sort command \
        --setup "sleep ${SLEEP_SECONDS}" \
        --warmup 1 \
        --parameter-list INDEX_COMMIT "${INDEX_COMMITS}" \
        "./bin/stv-rs-{INDEX_COMMIT} --arithmetic ${ARITHMETIC} --input ${INPUT} meek ${EQUALIZE} --parallel=rayon --num-threads=${NUM_THREADS} > /dev/null"
done
